...
func NewZipfGenerator(
	...
	if iMin > iMax {
		return nil, nil //errors.Errorf("iMin %d > iMax %d", iMin, iMax)
	}
	if theta < 0.0 || theta == 1.0 {
		return nil, nil //errors.Errorf("0 < theta, and theta != 1")
	}
	...
	if err != nil {
		return nil, nil //errors.Errorf("Could not compute zeta(2,theta): %s", err)
	}
	// fmt.Printf("jenndebug Eh! I'm computin here!\n")
	var zetaN float64
	zetaN, err = computeZetaFromScratch(iMax+1-iMin, theta)
	if err != nil {
		return nil, nil //errors.Errorf("Could not compute zeta(%d,theta): %s", iMax, err)
	}
...
func computeZetaIncrementally(oldIMax, iMax uint64, theta float64, sum float64) (float64, error) {
	if iMax < oldIMax {
		return 0, nil //errors.Errorf("Can't increment iMax backwards!")
	}
...
func computeZetaFromScratch(n uint64, theta float64) (float64, error) {
	...
	if err != nil {
		return zeta, nil //errors.Errorf("could not compute zeta: %s", err)
	}
	return zeta, nil
}
...
func (z *ZipfGenerator) IncrementIMax(count uint64) error {
	...
	if err != nil {
		z.zipfGenMu.mu.Unlock()
		return nil //errors.Errorf("Could not incrementally compute zeta: %s", err)
	}
